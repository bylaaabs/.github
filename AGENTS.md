...
- `infra/` is the map of everything and is a submodule here; `service.yaml` at the root describes this repo and must agree with `infra/map/services.yaml`.
- `CLAUDE.md` is just `@AGENTS.md`. edit this file, not that one.

## this repository

`bylaaabs/.github` is the public defaults repo of the organisation. github reads it: the org profile, the issue and pull request templates, the community files and the reusable workflows every other repo inherits or calls. it is not a product or a service, so it has no `service.yaml` and no `infra/` submodule.
